...
const areNodesEqual = (a: INestingToken, b: INestingToken) => a.collection_id === b.collection_id && a.token_id === b.token_id;
...
const sortTokensInBundle = (bundle: INestingToken) => {
  ...
  bundle.nestingChildren = bundle.nestingChildren?.sort((a, b) => a.token_id > b.token_id ? 1 : -1).map((token) => sortTokensInBundle(token));
  return bundle;
};
...
function BundleTreeSection({ onViewTokenDetails: onViewTokenDetailsProps, token }: IProps) {
  ...
  const { bundle: bundleQL, isBundleFetching } = useGraphQLBundleTree(token.collection_id, token.token_id);

  const onActionClick = useCallback((/* action: BundleModalType */) => (token: INestingToken) => {
  ...
  const onViewTokenDetails = useCallback((token: INestingToken) => {
    ...
    <>
      <BundlePagePaper>
        <HeaderStyled id={'bundle'}>
          ...
          <Text color='grey-500' size='m'>{tokensCount + 1} items total</Text>
        </HeaderStyled>
        <Content>
