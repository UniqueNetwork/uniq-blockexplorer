...
const LastBlocksComponent = ({
  ...
  loading,
  onPageChange,
}: BlockComponentProps<BlocksData>) => {
    ...
    <div>
      {deviceSize !== DeviceSize.sm && (
        <Table
          ...
          data={
            !loading && data?.view_last_block.length
              ? blocksWithTimeDifference(data?.view_last_block)
              : []
          }
      ...
      {deviceSize === DeviceSize.sm && (
        <div className={'table-sm'}>
          ...
          {!loading && data?.view_last_block.length === 0 && (
            <Text className={'text_grey'}>No data</Text>
          )}
          {!loading &&
            blocksWithTimeDifference(data?.view_last_block).map((item) => (
              <div key={`block-${item.block_number}`} className={'row'}>
                <div>
