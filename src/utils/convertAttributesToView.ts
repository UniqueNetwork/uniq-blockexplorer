...
const getValueOrDefaultValue = (
  obj: LocalizedStringOrBoxedNumberWithDefault,
  lang = 'en',
): string => {
  return (obj as LocalizedStringWithDefault)[lang] ?? obj._?.toString();
};

export const convertAttributesToView = (
  ...
  return Object.values(attributes).map((value: AttributeValue) => {
    if (value.isArray) {
      return {
        ...
        value: (value.value as LocalizedStringOrBoxedNumberWithDefault[]).map(
          (val: LocalizedStringOrBoxedNumberWithDefault) => getValueOrDefaultValue(val),
        ),
      };
    }
    ...
    return {
      name: getValueOrDefaultValue(value.name),
      value: getValueOrDefaultValue(
        value.value as LocalizedStringOrBoxedNumberWithDefault,
      ),
    };
  });
};
